...
load_metrics = torch.load("./metrics/apple_news_daily_summarized_spacy_ProsusAI_finbert_spacy_text_512_16_1e-05/4.pt")
...
plt.title('Training and Validation Accuracy - Apple Daily')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')

for a,b in zip(epochs, load_metrics['val_accuracy'] ): 
    plt.text(a , b , str(round(b,2)) , ha='left', va='bottom' , color = '#1f77b4')
    # plt.set_color('red')

for a,b in zip(epochs, load_metrics['train_accuracy'] ): 
    plt.text(a , b - 1, str(round(b,2)) , ha='right', va='bottom' , color = '#ff7f0e')
...
plt.title('Training and Validation Loss - Apple Daily')
plt.xlabel('Epochs')
plt.ylabel('Loss')

for a,b in zip(epochs, load_metrics['val_loss'] ): 
    plt.text(a , b , str(round(b,2)) , ha='center', va='top', color = '#1f77b4')

for a,b in zip(epochs, load_metrics['train_loss']): 
    plt.text(a , b, str(round(b,2)) , ha='center', va='bottom' , color = '#ff7f0e')
...
load_report = torch.load("./plots/apple_news_daily_summarized_spacy_ProsusAI_finbert_spacy_text_512_16_1e-05_report.pt")
# print(load_report)
